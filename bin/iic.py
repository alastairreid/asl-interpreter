...
Typical usage:
    ...
    mkdir genc
    bin/iic.py --output-dir=genc --basename=sim --intermediates=genc/log --line-info --num-c-files=8 > genc/spec.prj
    iii --batchmode --nobanner --project=genc/spec.prj --configuration=imports.json --configuration=exports.json spec.isa

Alternative usage (compile and run the ISA function 'main' using backend fallback):

    bin/iic.py --backend=fallback --run spec.isa
"""
...
base_script = """
// Autogenerated script to generate C code from Intel(R) ISA Specification Language
//
...
// This step deletes any bitwidth-polymorphic functions
// that have been completely replaced by specialized versions of the functions.
//
// The list of exported functions can be specified in two ways.
//
// 1) Using foreign export declarations. Example of exporting two functions
// called "Reset" and "Step":
//
...
// the type of an ISA function with a matching type and provide a configuration
...
//     }}
//
// If you have defined the behavior of these functions in ISA (e.g., for use in
// the ISA interpreter), you need to delete the ISA definitions of these
// functions.
...
// etc. know what file/line in the .isa file produced each line of C code.
...
def mk_filenames(backend, working_directory, basename, use_cxx):
    old_project_file = f"{working_directory}/asl2c.prj" # deprecated
    project_file = f"{working_directory}/{basename}.prj"
    ...
    return (old_project_file, project_file, config_file, c_files, exe_file)
...
def run_iii(iii, args, isa_files, project_file, configurations):
    ...
    for file in configurations:
        iii_cmd.append(f"--configuration={file}")
    iii_cmd.extend(isa_files)
    run(iii_cmd)
...
def main() -> int:
    ...
    parser.add_argument("--basename", help="basename of generated C files (default: \"isa\")", metavar="output_prefix", default="isa")
    ...
    parser.add_argument("--constraint-checks", help="perform type constraint checks", action=argparse.BooleanOptionalAction)
    parser.add_argument("--extra-c", help="extra C file to be compiled/linked with ISA code (C generation only)", action='append', default=[])
    ...
    parser.add_argument("--print-ld-flags", help="print the Linker flags needed to use the selected ISA C runtime", action=argparse.BooleanOptionalAction)
    parser.add_argument("--build", help="compile and link the ISA code", action='store_true')
    parser.add_argument("--run", help="compile, link and run the ISA code", action='store_true')
    parser.add_argument("--show-final-isa", help="stop after optimization, dump ISA", action=argparse.BooleanOptionalAction)
    ...
    args.build = args.build or args.run or args.show_final_isa
    if args.isa_files and not args.build:
    ...
    if args.backend == "interpreter" and not args.run:
        print("Error: must specify --run with iii backend")
        exit(1)
    ...
    if args.print_c_flags:
        ...
        working_directory = make_working_dir(args.working_dir, prefix="isatest.")
        config_file = f"{working_directory}/config.json"
        mlir_file = f"{working_directory}/isa.mlir"
        iii_cmd = [
            iii,
            "--batchmode",
        ...
        (old_project_file, project_file, config_filename, c_files, exe_file) = mk_filenames(backend, working_directory, args.basename, args.generate_cxx)
