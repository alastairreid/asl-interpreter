...
let constant_u32 (fmt : PP.formatter) (s : string) : unit = PP.pp_print_string fmt ("static_cast<int>(" ^ s ^ ")")
...
let kw_asl_int (fmt : PP.formatter) : unit = keyword fmt "ac_int<128,true>"
...
let minint_constant (fmt : PP.formatter) (n : int) : unit =
  constant fmt ("ac_int<128,true>(value<AC_VAL_MIN,"^ string_of_int n ^",true>(0))")

(* Generate ASL_INT<n>_MAX macro constant *)
let maxint_constant (fmt : PP.formatter) (n : int) : unit =
  constant fmt ("ac_int<128,true>(value<AC_VAL_MAX,"^ string_of_int n ^",true>(0))")
...
let int_literal_fit_int64 (fmt : PP.formatter) (x : Z.t) : unit =
  int_constant fmt 64 x (fun fmt x -> constant fmt (Z.format "%d" x ))
...
let int_literal_not_fit_int64 (fmt : PP.formatter) (x : Z.t) : unit =
  ...
  int_constant fmt num_bits x (fun fmt x ->
      ...
      (* keyword fmt ("ac::bit_fill< ac_int<" ^ string_of_int (num_bits) ^ ",false>>((int [" ^ string_of_int num_limbs ^ "])"); *)
      keyword fmt ("ac::bit_fill< ac_int<" ^ string_of_int (num_bits) ^ ",true>>");
      parens fmt (fun _ ->
        parens fmt (fun _ -> keyword fmt ("int [" ^ string_of_int num_limbs ^ "]"));
        braces fmt (fun _ -> commasep fmt (constant_u32 fmt) (limbs));
      );
...
let bitsLit (fmt : PP.formatter) (x : AST.bitsLit) : unit =
  ...
  let bit_to_hex (s : string) : string =
    Z.format "%#x" (Z.of_string_base 2 s)
  in
  ...
  if len <= 64 then constant fmt (bit_to_hex x)
    ...
    in
    keyword fmt ("ac::bit_fill< ac_int<" ^ string_of_int (len) ^ ",false>>((int [" ^ string_of_int num_limbs ^ "])");
    braces fmt (fun _ -> commasep fmt (constant_u32 fmt) (limbs));
    delimiter fmt ")"
...
let bits (fmt : PP.formatter) (width : int) : unit =
  keyword fmt ("ac_int<" ^ string_of_int (width) ^ ",false>")
...
and make_unop_post (fmt : PP.formatter) (op : unit -> unit) (x : unit -> unit) : unit
    =
  parens fmt (fun _ ->
    x ();
    op ())
...
and unop_post (loc : Loc.t) (fmt : PP.formatter) (op : string) (args : AST.expr list) : unit =
  match args with
  | [ x ] -> make_unop_post fmt (fun _ -> delimiter fmt op) (fun _ -> expr loc fmt x)
  | _ -> raise (Error.Unimplemented (loc, "unop: " ^ op, fun fmt -> ()))
...
and apply_ac_int_builtin (loc : Loc.t) (fmt : PP.formatter) (f : unit -> unit)
    (widths : AST.expr list) (args : AST.expr list) : unit =
...
and cast (fmt : PP.formatter) (f : string) (signed : string) (x : unit -> unit) : unit =
  delimiter fmt ("ac_int<" ^ f ^ ","^ signed^">");
  parens fmt x;

and sign_extend_bits (loc : Loc.t) (fmt : PP.formatter) (target_width : int) (x : AST.expr) :
    unit =
  cast fmt (string_of_int target_width) "true" (fun _ -> expr loc fmt x)
...
and funcall (loc : Loc.t) (fmt : PP.formatter) (f : Ident.t) (tes : AST.expr list)
  ...
  | _ when Ident.equal f fdiv_int ->
    binop loc fmt "/" args
  | _ when Ident.equal f frem_int ->
    binop loc fmt "%" args
  ...
  | _ when Ident.equal f eq_bits -> binop loc fmt "==" args
  ...
  | _ when Ident.equal f sub_bits -> binop loc fmt "-" args
  ...
  | _ when Ident.equal f not_bits -> unop_post loc fmt ".bit_complement()" args
  ...
  | [ x; n ] when Ident.equal f replicate_bits ->
      ...
      let m = List.hd tes in
      let nm = Asl_utils.mk_litint (const_int_expr loc n * const_int_expr loc m) in
      keyword fmt ("asl::replicate<" ^ string_of_int (const_int_expr loc nm) ^ ">");
      parens fmt (fun _ ->
        expr loc fmt n;
        comma fmt;
        expr loc fmt x)
...
and lslice (loc : Loc.t) (fmt : PP.formatter) (t : AST.ty) (e : AST.lexpr)(r : AST.expr)
  (s : AST.slice) : unit =
    match s with
      | Slice_LoWd (lo, wd) ->(
          lexpr loc fmt e;
...
let lexpr_assign (loc : Loc.t) (fmt : PP.formatter) (x : AST.lexpr) (r : AST.expr) : unit =
  match x with
  | LExpr_Slices (t, l, s) ->
      List.iter (lslice loc fmt t l r) s;
  | LExpr_Wildcard ->
...
(* convert from internal representation to C *)
let ffi_from_c (loc : Loc.t) (fmt : PP.formatter) (arg : Ident.t) (t : AST.ty) : unit =
    ( match t with
    | Type_Integer _ ->
        Format.fprintf fmt "%a.to_int()" ident arg
    | _ ->
        let msg = Format.asprintf "Type '%a' cannot be used in functions that are imported or exported between ASL and C"
          FMT.ty t
        in
...
(* convert from internal representation to C *)
let ffi_to_c (loc : Loc.t) (fmt : PP.formatter) (pp : PP.formatter -> unit) (t : AST.ty) : unit =
    ( match t with
    | Type_Integer _ ->
        Format.fprintf fmt "(%a).to_int()" (fun fmt _ -> pp fmt) ()
    | _ ->
        let msg = Format.asprintf "Type '%a' cannot be used in functions that are imported or exported between ASL and C"
          FMT.ty t
        in
...
let mk_ffi_defn (fmt : PP.formatter) ((function_name, fty, loc) : (string * AST.function_type * Loc.t)) : unit =
    ...
    | Some rty ->
        ...
        ffi_to_c loc fmt (fun fmt -> Format.pp_print_string fmt "__r") rty;
        Format.fprintf fmt ";";
    | None ->
        ()
    );
...
let mk_ffi_exports (decls : AST.declaration list) (function_names : string list) :
  ...
  let mk_protos fmt = cutsep fmt (mk_ffi_proto fmt) infos in
  let mk_defns fmt = cutsep fmt (mk_ffi_defn fmt) infos in
  (mk_protos, mk_defns)
...
let emit_cpp_header (dirname : string) (basename : string)
    (sys_h_filenames : string list) (h_filenames : string list)
    (f : PP.formatter -> unit) : unit =
  let basename = basename ^ ".hpp" in
...
let generate_files (num_c_files : int) (dirname : string) (basename : string)
  ...
  let sys_h_filenames = [ "cstdint" ] in
  let h_filenames = [ "asl/runtime.hpp"; "ac_int.h" ] in

  let basename_t = basename ^ "_types" in
  emit_cpp_header dirname basename_t sys_h_filenames h_filenames (fun fmt ->
  ...
  emit_cpp_header dirname basename_e sys_h_filenames h_filenames (fun fmt ->
      exceptions fmt ds
  );
  let basename_v = basename ^ "_vars" in
  emit_cpp_header dirname basename_v sys_h_filenames h_filenames (fun fmt ->
      extern_declarations fmt (var_decls ds)
  );

  let gen_h_filenames =
    List.map (fun s -> s ^ ".hpp") [ basename_t; basename_e; basename_v ]
  in

  let filename_e = Filename.concat dirname basename_e in
  emit_cpp_source filename_e gen_h_filenames (fun fmt ->
      exceptions_init fmt);

  let filename_v = Filename.concat dirname basename_v in
  emit_cpp_source filename_v gen_h_filenames (fun fmt ->
  ...
  let emit_funs ?(index : int option) (ds : AST.declaration list) : unit =
    emit_cpp_source filename_f ?index gen_h_filenames (fun fmt ->
        declarations fmt ds)
  in
  if num_c_files = 1 then
    emit_cpp_source filename_f gen_h_filenames (fun fmt ->
       declarations fmt ds;
      Format.fprintf fmt "@,";
      ffi_definitions fmt
    )
    ...
    let rec emit_funs_by_chunk (i : int) (acc : AST.declaration list) = function
      ...
      | l when i = num_c_files ->
          emit_cpp_source filename_f ~index:i gen_h_filenames (fun fmt ->
...
(****************************************************************
 * Command: :generate_cpp
 ****************************************************************)

let _ =
  let opt_dirname = ref "" in
  let opt_num_c_files = ref 1 in
  let opt_basename = ref "asl2cpp" in
  ...
  let flags = Arg.align [
        ...
        ("--runtime",      Arg.Symbol (runtimes, set_runtime), "ac     Select runtime system");
  ...
  in
  Commands.registerCommand "generate_cpp" flags [] [] "Generate CPP" cmd
