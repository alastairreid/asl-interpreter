...
class hoist_lets (ds : AST.declaration list option) = object (self)
    ...
    val mutable bindings : Isa_utils.binding list = []
    val mutable checks : Isa_utils.check list = []

    (* Hoist let-expressions and assertions out of an expression *)
    method hoist_lets_out_of_expression (x : AST.expr) : (Isa_utils.binding list * Isa_utils.check list * AST.expr) =
      let old_bindings = bindings in
      let old_checks = checks in
      bindings <- [];
      checks <- [];
      let x' = visit_expr (self :> isaVisitor) x in
    ...
    method hoist_lets_to_expression_top (x : AST.expr) : AST.expr =
      ...
      Isa_utils.mk_let_exprs lets (Isa_utils.mk_assert_exprs asserts x')

    method! vexpr x =
      ( match x with
      | Expr_Let (x, ty, e1, e2) ->
          let e1' = visit_expr (self :> isaVisitor) e1 in
          let e2' = visit_expr (self :> isaVisitor) e2 in
      ...
      | Expr_Assert (e1, e2, loc) ->
          ...
          Visitor.ChangeTo e2'
      | Expr_TApply (f, [], [a; b], NoThrow) when Ident.in_list f [lazy_and_bool; lazy_or_bool] ->
          let a' = visit_expr (self :> isaVisitor) a in
    ...
    method! vstmt x =
      ...
      ( match x with
      | Stmt_If ([], (e, el), loc) ->
          let e' = visit_stmts (self :> isaVisitor) e in
          Visitor.ChangeTo e'
      | Stmt_If ((c, t, l)::els, (e, el), loc) ->
          let (lets, asserts, c') = self#hoist_lets_out_of_expression c in
          let t' = visit_stmts (self :> isaVisitor) t in
          let els' = Visitor.mapNoCopy (visit_s_elsif (self :> isaVisitor)) els in
          let e' = visit_stmts (self :> isaVisitor) e in
      ...
      | Stmt_While (c, b, loc) ->
          ...
          let b' = visit_stmts (self :> isaVisitor) b in
          if c == c' && b == b' then (
            Visitor.SkipChildren
          ) else (
...
let xform_decls (ds : AST.declaration list) : AST.declaration list =
  ...
  List.map (Isa_visitor.visit_decl (transformer :> Isa_visitor.isaVisitor)) ds
