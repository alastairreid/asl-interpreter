            ...
            if r = fail then (
                ...
                if r.live = Some false then begin
                    Format.printf "Warning (%s): Dead code detected '%a'\n"
                        (Loc.to_string (Isa_utils.stmt_loc y))
...
let rec check_expression_order (loc : Loc.t) (effects : effects_class) (e : AST.expr) : (IdentSet.t * IdentSet.t * bool) =
  ...
  | _ ->
    let ordered = (* is evaluation order specified by ISA language? *)
      ( match e with
      | Expr_If _ -> true
  ...
  object
    ...
    method! vexpr x =
      ...
      if !check_call_markers then begin
          ignore (Isa_visitor.visit_expr (new exn_call_checks_class markers loc :> Isa_visitor.isaVisitor) x)
      end else begin
          ignore (Isa_visitor.visit_expr (new rethrow_checks_class effects loc) x)
      end;
      SkipChildren

    method! vstmt x =
      if !check_call_markers then begin
          ignore (Isa_visitor.visit_stmt (new exn_call_checks_class markers loc :> Isa_visitor.isaVisitor) x)
      end else begin
          ignore (Isa_visitor.visit_stmt (new rethrow_checks_class effects (stmt_loc x)) x)
      end;
      DoChildren
  end
...
class global_checks_class_wrapper (effects : effects_class) (markers : (AST.can_throw * Loc.t) Bindings.t) =
  object
    ...
    method! vstmt x =
      ignore (Isa_visitor.visit_stmt (new global_checks_class effects markers (stmt_loc x)) x);
      SkipChildren
  end
...
let check_decls (ds : AST.declaration list) : AST.declaration list =
  ...
  let check_decl d =
    ( try
        ignore (Isa_visitor.visit_decl checker d);
        if !check_defn_markers then begin
            ignore (Isa_visitor.visit_decl exn_defn_checker d)
        end
    with
