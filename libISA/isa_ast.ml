...
and expr =
 ...
 | Expr_UApply of expr * bool * (Ident.t option * expr) list * can_throw (* function call before typechecking - optional named arguments, no type parameters *)
 ...
 | Expr_Slice of slice (* first class slices *)
...
and ty =
 | Type_Integer of set_range list option
...
and set_range =
   Set_Single of expr
 | Set_Range of expr option * expr option (* at most one is None *)
...
declaration =
 ...
 | Decl_Use of Ident.t * Ident.t * Loc.t
...
let higherPriorityThan (x: binop) (y: binop): bool option =
    ...
    else if x = Binop_Power    && y = Binop_Remainder then Some(true)
    ...
    else if y = Binop_Power    && x = Binop_Remainder then Some(false)
...
let rec mk_binop_chain (op : binop) (e : expr) (es : expr list) : expr =
    ( match es with
    | (e'::es') -> mk_binop_chain op (Expr_Binop (e, op, e')) es'
    | [] -> e
    )

let rec mk_binop_chain2 (e : expr) (es : (binop * expr) list) : expr =
    ( match es with
    | ((op, e')::es') -> mk_binop_chain2 (Expr_Binop (e, op, e')) es'
    | [] -> e
    )
