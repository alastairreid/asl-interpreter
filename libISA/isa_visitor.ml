...
(** For each datatype in the ISA AST, a visitor defines what actions
...
(** The following set of recursive functions are the ISA specific
...
let rec visit_exprs (vis : isaVisitor) (xs : expr list) : expr list =
  mapNoCopy (visit_expr vis) xs

and visit_args (vis : isaVisitor) (xs : (Ident.t option * expr) list) : (Ident.t option * expr) list =
  mapNoCopy (visit_arg vis) xs

and visit_arg (vis : isaVisitor) (x : Ident.t option * expr) : (Ident.t option * expr) =
  let (name, e) = x in
  let e' = visit_expr vis e in
  if e == e' then x else (name, e')

and visit_var (vis : isaVisitor) (kind : access_kind) (x : Ident.t) : Ident.t =
  let aux (_ : isaVisitor) (x : Ident.t) : Ident.t = x in
  doVisit vis (vis#vvar kind x) aux x

and visit_lvar (vis : isaVisitor) (x : Ident.t) : Ident.t =
  let aux (_ : isaVisitor) (x : Ident.t) : Ident.t = x in
  doVisit vis (vis#vlvar x) aux x

and visit_e_elsif (vis : isaVisitor) (x : e_elsif) : e_elsif =
  let aux (vis : isaVisitor) (x : e_elsif) : e_elsif =
    ( match x with
    | (c, e) ->
...
and visit_slice (vis : isaVisitor) (x : slice) : slice =
  let aux (vis : isaVisitor) (x : slice) : slice =
    match x with
    | Slice_Single e ->
        let e' = visit_expr vis e in
...
and visit_change_part (vis : isaVisitor) (x : change * expr) : change * expr =
  let (c, e) = x in
  let c' = visit_change vis c in
  let e' = visit_expr vis e in
  if c == c' && e == e' then x else (c', e')

and visit_change (vis : isaVisitor) (x : change) : change =
  let aux (vis : isaVisitor) (x : change) : change =
    ( match x with
    | Change_Field f -> x
    | Change_Slices ss ->
...
and visit_patterns (vis : isaVisitor) (xs : pattern list) : pattern list =
  mapNoCopy (visit_pattern vis) xs

and visit_pattern (vis : isaVisitor) (x : pattern) : pattern =
  let aux (vis : isaVisitor) (x : pattern) : pattern =
    match x with
    | Pat_Lit _ -> x
    | Pat_Const c ->
...
and visit_expr (vis : isaVisitor) (x : expr) : expr =
  let aux (vis : isaVisitor) (x : expr) : expr =
    ...
    | Expr_UApply (f, use_array_syntax, args, throws) ->
        let f' = visit_expr vis f in
        let args' = visit_args vis args in
        if f == f' && args == args' then x
        else Expr_UApply (f', use_array_syntax, args', throws)
    | Expr_Tuple es ->
        let es' = visit_exprs vis es in
    ...
    | Expr_Slice s ->
        let s' = visit_slice vis s in
        if s == s' then x else Expr_Slice s
    | Expr_Lit _ -> x
    | Expr_AsType (e, t) ->
        let e' = visit_expr vis e in
        let t' = visit_type vis t in
...
and visit_fieldassignment (vis : isaVisitor) (x : Ident.t * expr) : Ident.t * expr =
  match x with
  | f, e ->
      let e' = visit_expr vis e in
      if e == e' then x else (f, e')

and visit_set_range (vis : isaVisitor) (x : set_range) :
    set_range =
  let aux (vis : isaVisitor) (x : set_range) : set_range =
    match x with
    | Set_Single e ->
        let e' = visit_expr vis e in
        if e == e' then x else Set_Single e'
    | Set_Range (lo, hi) ->
        let lo' = mapOptionNoCopy (visit_expr vis) lo in
        let hi' = mapOptionNoCopy (visit_expr vis) hi in
        if lo == lo' && hi == hi' then x else Set_Range (lo', hi')
  in
  doVisit vis (vis#vset x) aux x

and visit_sets (vis : isaVisitor) (x : set_range list) :
    set_range list =
  mapNoCopy (visit_set_range vis) x

and visit_types (vis : isaVisitor) (xs : ty list) : ty list =
  mapNoCopy (visit_type vis) xs

and visit_type (vis : isaVisitor) (x : ty) : ty =
  let aux (vis : isaVisitor) (x : ty) : ty =
    match x with
    | Type_Integer ocrs ->
        let ocrs' = mapOptionNoCopy (visit_sets vis) ocrs in
...
let rec visit_lexprs (vis : isaVisitor) (xs : lexpr list) : lexpr list =
  mapNoCopy (visit_lexpr vis) xs

and visit_lexpr (vis : isaVisitor) (x : lexpr) : lexpr =
  let aux (vis : isaVisitor) (x : lexpr) : lexpr =
    match x with
    | LExpr_Wildcard -> x
    | LExpr_Var v ->
...
let visit_decl_bit (vis : isaVisitor) (x : (Ident.t option * ty)) : (Ident.t option * ty) =
  ( match x with
  | (ov, ty) ->
...
let rec visit_decl_item (vis : isaVisitor) (x : decl_item) : decl_item =
  match x with
  | DeclItem_Var (v, oty) ->
      let v' = visit_lvar vis v in
 ...
 *)
let rec visit_stmts (vis : isaVisitor) (xs : stmt list) : stmt list =
  with_locals vis (List.concat_map locals_of_stmt xs) (mapNoCopyList (visit_stmt vis)) xs

and visit_stmt (vis : isaVisitor) (x : stmt) : stmt list =
  let aux (vis : isaVisitor) (x : stmt) : stmt =
    match x with
...
and visit_s_elsif (vis : isaVisitor) (x : s_elsif) : s_elsif =
  let aux (vis : isaVisitor) (x : s_elsif) : s_elsif =
    ( match x with
    | (c, b, loc) ->
        let c' = visit_expr vis c in
        let b' = visit_stmts vis b in
...
and visit_alt (vis : isaVisitor) (x : alt) : alt =
  let aux (vis : isaVisitor) (x : alt) : alt =
    match x with
    | Alt_Alt (ps, oc, b, loc) ->
        let ps' = visit_patterns vis ps in
...
and visit_catcher (vis : isaVisitor) (x : catcher) : catcher =
  let aux (vis : isaVisitor) (x : catcher) : catcher =
    match x with
...
let visit_parameter (vis : isaVisitor) (x : Ident.t * ty option) :
    Ident.t * ty option =
  let (v, oty) = x in
  let oty' = mapOptionNoCopy (visit_type vis) oty in
  let v' = visit_var vis Definition v in
...
let visit_parameters (vis : isaVisitor) (xs : (Ident.t * ty option) list) :
    (Ident.t * ty option) list =
  mapNoCopy (visit_parameter vis) xs

let visit_arg_no_default (vis : isaVisitor) (x : Ident.t * ty) : (Ident.t * ty) =
  let (v, ty) = x in
  let v' = visit_var vis Definition v in
  let ty' = visit_type vis ty in
  if v == v' && ty == ty' then x else (v', ty')

let visit_args_no_default (vis : isaVisitor) (xs : (Ident.t * ty) list) : (Ident.t * ty) list =
  mapNoCopy (visit_arg_no_default vis) xs

let visit_arg (vis : isaVisitor) (x : Ident.t * ty * expr option) : (Ident.t * ty * expr option) =
  let (v, ty, od) = x in
  let v' = visit_var vis Definition v in
  let ty' = visit_type vis ty in
...
let visit_args (vis : isaVisitor) (xs : (Ident.t * ty * expr option) list) : (Ident.t * ty * expr option) list =
  mapNoCopy (visit_arg vis) xs
...
let visit_funtype (vis : isaVisitor) (locals : Ident.t list) (fty : function_type) : function_type =
  ...
  then fty
    ...
    throws=fty.throws;
    is_builtin=fty.is_builtin;
  }

let visit_decl (vis : isaVisitor) (x : declaration) : declaration =
  let aux (vis : isaVisitor) (x : declaration) : declaration =
    match x with
    ...
    | Decl_Use (p1, p2, loc) ->
        let p1' = visit_var vis Definition p1 in
        let p2' = visit_var vis Definition p2 in
        if p1 == p1' && p2 == p2' then x else Decl_Use(p1', p2', loc)
  in

  doVisit vis (vis#vdecl x) aux x

(****************************************************************)
(** {2 nopIsaVisitor class}                                     *)
(****************************************************************)

(** The nopIsaVisitor class defines a visitor that recursively
...
class nopIsaVisitor : isaVisitor =
  object
    ...
    method vset (_ : set_range) = DoChildren
