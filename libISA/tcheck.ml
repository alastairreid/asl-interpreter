...
(** Type inference and checker for ISA language *)

module AST = Isa_ast
module Visitor = Isa_visitor
module FMT = Isa_fmt
...
module GlobalEnv : sig
  ...
  val mkempty : unit -> t
  val clone : t -> t
  val addUses : t -> (Ident.t * Ident.t) list -> unit
  val addType : t -> Loc.t -> Ident.t -> typedef -> unit
  val renameType : t -> Ident.t -> Ident.t
  ...
  type t = {
    mutable uses : Ident.t Bindings.t;
  ...
  (* Add 'use x as y;' renamings - which rename y as x. *)
  let addUses (env : t) (uses : (Ident.t * Ident.t) list) : unit =
    env.uses <- add_bindings env.uses uses

  (* Rename x if we have a 'use' clause for it *)
  let apply_use (env : t) (x : Ident.t) : Ident.t =
    Bindings.find_opt x env.uses |> Option.value ~default:x

  let addType (env : t) (loc : Loc.t) (qid : Ident.t) (t : typedef) : unit =
    let qid = apply_use env qid in
  ...
  let renameType (env : t) (qid : Ident.t) : Ident.t =
    apply_use env qid
  ...
  let addFuns (env : t) (loc : Loc.t) (qid : Ident.t) (ftys : funtype list) : unit =
    let qid = apply_use env qid in
  ...
  let addOperators1 (env : t) (loc : Loc.t) (op : AST.unop) (funs : funtype list) : unit =
  ...
  let addOperators2 (env : t) (loc : Loc.t) (op : AST.binop) (funs : funtype list) : unit =
...
(** Convert ISA expression to Z3 expression.
    This only copes with a limited set of operations:
    ==, +, -, negate, *, and /.
    (It is possible that we will need to extend this list in the future but
    it is sufficient for the current ISA specifications.)

    The support for "/" is not sound - it is a hack needed to cope with
    the way ISA code is written and generally needs a side condition
...
let rec z3_of_expr
  ...
  (* todo: the following lines involving "/" are not sound *)
  | Expr_TApply
      ( i,
        [],
    ...
    when Ident.equal i mul_int && Ident.equal j pdiv_int && b = c ->
  ...
  | Expr_TApply
    ...
    when Ident.equal i mul_int && Ident.equal j pdiv_int && a = c ->
  ...
  | Expr_TApply
    ...
    when Ident.equal i add_int && Ident.equal j pdiv_int && Ident.equal k pdiv_int && a1 = a2 && b1 = b2 && b1 = two ->
  ...
  | Expr_TApply
      ( i,
        ...
        [ a; Expr_TApply (j, [], [ b; c ], _) ],
        _
      ) when Ident.equal i eq_int && Ident.equal j pdiv_int ->
  ...
  | Expr_TApply (i, [], [ a; b ], _) when Ident.equal i pdiv_int ->
      Z3.Arithmetic.mk_div ctx (z3_of_expr ctx ufs a) (z3_of_expr ctx ufs b)
  ...
  | Expr_TApply (i, [], [ a ], _) when Ident.equal i not_bool ->
    Z3.Boolean.mk_not ctx (z3_of_expr ctx ufs a)
  | _ ->
      if !verbose then
        Format.fprintf fmt
...
let simplify_constraints (cs : set_range list) : set_range list =
  (* todo: remove constants, duplicates, etc. *)
  cs

let constraint_union (ocrs1 : set_range list option) (ocrs2 : set_range list option) : set_range list option =
...
let is_legal_constraint_range (cr : AST.set_range) : AST.expr option =
  ( match cr with
  | Set_Single _ -> None
  | Set_Range (Some lo, Some hi) -> Some (mk_le_int lo hi)
  | _ -> None
  )
...
let lift_predicate (d : 'a) (f : 'a -> 'a -> 'a) (ox : 'a option) (oy : 'a option) : 'a option =
  ( match (ox, oy) with
  | (Some x, Some y) -> Some (f x y)
  | _ -> Some d
  )

let mk_and' = lift2 mk_and
let mk_or' = lift2 mk_or
let mk_le_int' = lift_predicate asl_true mk_le_int
let mk_add_int' = lift2 mk_add_int
let mk_sub_int' = lift2 mk_sub_int
let mk_mul_int' = lift2 mk_mul_int
let mk_neg_int' = Option.map mk_neg_int
let mk_min_int' = lift2 mk_min_int
let mk_max_int' = lift2 mk_max_int
let mk_pow_int_int' = lift2 mk_pow_int_int
let mk_exact_div_int' = lift2 mk_exact_div_int

let are_legal_constraint_ranges (crs : AST.set_range list) : AST.expr =
  mk_ands (List.filter_map is_legal_constraint_range crs)

let constraint_range_to_expr (v : AST.expr) (cr : AST.set_range) : AST.expr =
  ( match cr with
  | Set_Single e -> if v = e then asl_true else mk_eq_int v e
  | Set_Range (lo, hi) -> Option.get (mk_and' (mk_le_int' lo (Some v)) (mk_le_int' (Some v) hi))
  )

let constraint_ranges_to_expr (v : AST.expr) (crs : AST.set_range list) : AST.expr =
  mk_ors (List.map (constraint_range_to_expr v) crs)

let is_subrange (env : Env.t) (crs1 : set_range list) (crs2 : set_range list) : bool =
  let v = Expr_Var wildcard_ident in
  let chk = mk_or
              (mk_not (mk_and (are_legal_constraint_ranges crs1) (constraint_ranges_to_expr v crs1)))
...
let check_subrange_satisfies (env : Env.t) (loc : Loc.t) (ocrs1 : set_range list option) (ocrs2 : set_range list option) : unit =
...
let rec check_subtype_satisfies (env : Env.t) (loc : Loc.t) (ty1 : AST.ty) (ty2 : AST.ty) : unit =
  ...
  | Type_Constructor (tc1, es1), Type_Constructor (tc2, es2) when Ident.equal tc1 tc2 ->
...
let rec least_supertype (env : Env.t) (loc : Loc.t) (ty1 : AST.ty) (ty2 : AST.ty) : AST.ty =
  ...
  | (Type_Array (ixty1, elty1), Type_Array (ixty2, elty2)) ->
      ( match (ixty1, ixty2) with
      | (Index_Enum tc1, Index_Enum tc2) when Ident.equal tc1 tc2 -> ()
      | (Index_Int sz1, Index_Int sz2) -> check_equality env loc sz1 sz2
      | _ ->
        raise (DoesNotMatch (loc, "array index type", pp_ixtype ixty1, pp_ixtype ixty2))
      );
...
let mk_constraint_check (loc : Loc.t) (lets : binding list ref) (asserts : check list ref) (crs : AST.set_range list) (x : AST.expr) : AST.expr =
  if not !enable_runtime_checks then
      x
  else
      let x' = mk_expr_safe_to_replicate lets x type_integer in
...
(*   (eg if "x : Bits(8)", then, in a call "Is_Zero(x)", we     *)
...
let rec synthesize_type
  ...
  | Type_Constructor (tc1, es1), Type_Constructor (tc2, es2) when Ident.equal tc1 tc2 ->
      (if (List.length es1 <> List.length es2) then Format.printf "Mismatched args of type constructor %a@." Ident.pp tc1);
      assert (List.length es1 = List.length es2);
...
let lift1_constraints
    (f : set_range -> set_range)
    (oxs : set_range list option)
  : set_range list option
  =
...
let lift2_constraints
    (f : set_range -> set_range -> set_range)
    (oxs : set_range list option)
    (oys : set_range list option)
  : set_range list option
  =
  ( match (oxs, oys) with
...
let add_constraints (x : set_range) (y : set_range) : set_range =
  ( match (x, y) with
  | (Set_Single x, Set_Single y) -> Set_Single (mk_add_int x y)
  | (Set_Single x, Set_Range (yl,yh)) -> Set_Range (mk_add_int' (Some x) yl, mk_add_int' (Some x) yh)
  | (Set_Range (xl,xh), Set_Single y) -> Set_Range (mk_add_int' xl (Some y), mk_add_int' xh (Some y))
  | (Set_Range (xl,xh), Set_Range (yl,yh)) -> Set_Range (mk_add_int' xl yl, mk_add_int' xh yh)
  )

let sub_constraints (x : set_range) (y : set_range) : set_range =
  ( match (x, y) with
  | (Set_Single x, Set_Single y) -> Set_Single (mk_sub_int x y)
  | (Set_Single x, Set_Range (yl,yh)) -> Set_Range (mk_sub_int' (Some x) yl, mk_sub_int' (Some x) yh)
  | (Set_Range (xl,xh), Set_Single y) -> Set_Range (mk_sub_int' xl (Some y), mk_sub_int' xh (Some y))
  | (Set_Range (xl,xh), Set_Range (yl,yh)) -> Set_Range (mk_sub_int' xl yh, mk_sub_int' xh yl)
  )

let neg_constraints (x : set_range) : set_range =
  ( match x with
  | Set_Single x -> Set_Single (mk_neg_int x)
  | Set_Range (xl,xh) -> Set_Range (mk_neg_int' xh, mk_neg_int' xl)
  )

let mul_constraints (x : set_range) (y : set_range) : set_range =
  ( match (x, y) with
  | (Set_Single x, Set_Single y) -> Set_Single (mk_mul_int x y)
  | (Set_Single x, Set_Range (yl,yh)) ->
    let l = mk_min_int' (mk_mul_int' (Some x) yl) (mk_mul_int' (Some x) yh) in
    let h = mk_max_int' (mk_mul_int' (Some x) yl) (mk_mul_int' (Some x) yh) in
    Set_Range (l, h)
  | (Set_Range (xl, xh), Set_Single y) ->
    let l = mk_min_int' (mk_mul_int' (Some y) xl) (mk_mul_int' (Some y) xh) in
    let h = mk_max_int' (mk_mul_int' (Some y) xl) (mk_mul_int' (Some y) xh) in
    Set_Range (l, h)
  | (Set_Range (xl, xh), Set_Range (yl, yh)) ->
...
let pow_constraints (x : set_range) (y : set_range) : set_range =
  ( match (x, y) with
  | (Set_Single x, Set_Single y) -> Set_Single (mk_pow_int_int x y)
  | (Set_Single x, Set_Range (yl,yh)) ->
    let l = mk_pow_int_int' (Some x) yl in
    let h = mk_pow_int_int' (Some x) yh in
    Set_Range (l, h)
  | (Set_Range (xl, xh), Set_Single y) ->
    let l = mk_pow_int_int' xl (Some y) in
    let h = mk_pow_int_int' xh (Some y) in
    Set_Range (l, h)
  | (Set_Range (xl, xh), Set_Range (yl, yh)) ->
...
let exact_div_constraints (x : set_range) (y : set_range) : set_range =
  ( match (x, y) with
  | (Set_Single x, Set_Single y) -> Set_Single (mk_exact_div_int x y)
  | (Set_Single x, Set_Range (yl,yh)) ->
    let l = mk_exact_div_int' (Some x) yh in
    let h = mk_exact_div_int' (Some x) yl in
    Set_Range (l, h)
  | (Set_Range (xl, xh), Set_Single y) ->
    let l = mk_exact_div_int' xl (Some y) in
    let h = mk_exact_div_int' xh (Some y) in
    Set_Range (l, h)
  | (Set_Range (xl, xh), Set_Range (yl, yh)) ->
...
let max_constraints (x : set_range) (y : set_range) : set_range =
  ( match (x, y) with
  | (Set_Single x, Set_Single y) -> Set_Single (mk_max_int x y)
  | (Set_Single x, Set_Range (yl,yh)) -> Set_Range (mk_max_int' (Some x) yl, mk_max_int' (Some x) yh)
  | (Set_Range (xl,xh), Set_Single y) -> Set_Range (mk_max_int' xl (Some y), mk_max_int' xh (Some y))
  | (Set_Range (xl,xh), Set_Range (yl,yh)) -> Set_Range (mk_max_int' xl yl, mk_max_int' xh yh)
  )

let min_constraints (x : set_range) (y : set_range) : set_range =
  ( match (x, y) with
  | (Set_Single x, Set_Single y) -> Set_Single (mk_min_int x y)
  | (Set_Single x, Set_Range (yl,yh)) -> Set_Range (mk_min_int' (Some x) yl, mk_min_int' (Some x) yh)
  | (Set_Range (xl,xh), Set_Single y) -> Set_Range (mk_min_int' xl (Some y), mk_min_int' xh (Some y))
  | (Set_Range (xl,xh), Set_Range (yl,yh)) -> Set_Range (mk_min_int' xl yl, mk_min_int' xh yh)
  )
...
let mk_pow2_int (x : AST.expr) : AST.expr =
  ( match x with
  | Expr_Lit (VInt i) -> Isa_utils.mk_litbigint (Primops.prim_pow2_int i)
...
let refine_type (fty : funtype) (tys : AST.ty list) : AST.ty option =
  ...
  | (i, [Type_Bits (n, _)]) when Ident.equal i cvt_bits_uint -> Some (Type_Integer (Some [Set_Range (Some zero, Some (mk_sub_int (mk_pow2_int n) one))]))
  | (i, [Type_Bits (n, _)]) when Ident.equal i cvt_bits_sint -> Some (Type_Integer (Some [Set_Range (Some (mk_neg_int (mk_pow2_int (mk_sub_int n one))), Some (mk_sub_int (mk_pow2_int (mk_sub_int n one)) one))]))
...
let apply_with_runtime_checks (loc : Loc.t) (fty : fun_instance) (es : AST.expr list) (throws : can_throw) : AST.expr =
  let lets = ref [] in
  let asserts = ref [] in
  let r =
    ( match es with
...
(*                                                              *)
(* Note that disambiguation of assignment functions is          *)
...
let rec eq_structural (env : GlobalEnv.t) (loc : Loc.t) (ty1 : AST.ty) (ty2 : AST.ty) : bool =
  ...
  | Type_Constructor (c1, es1), Type_Constructor (c2, es2) -> Ident.equal c1 c2
  | Type_OfExpr e1, Type_OfExpr e2 -> raise (InternalError
      (loc, "eq_structural: typeof", (fun fmt -> FMT.ty fmt ty1), __LOC__))
  | Type_Array (ixty1, elty1), Type_Array (ixty2, elty2) ->
      ( match (ixty1, ixty2) with
      | Index_Enum tc1, Index_Enum tc2 -> Ident.equal tc1 tc2
...
and tc_expr (env : Env.t) (loc : Loc.t) (x : AST.expr) : AST.expr * AST.ty =
  ...
  | Expr_Binop (x, op, y) ->
      ...
      (apply_with_runtime_checks loc fty [x'; y'] NoThrow, fty.rty)
  | Expr_Field (e, f) ->
      let e', ty = tc_expr env loc e in
  ...
  | Expr_Slices (_, e, ss) -> (
       ...
       * First check for functions then check for arrays or bitvectors because
       * of conflicting names like SPSR and SPSR[] in the v8-A specification.
       *)

      (* variable slice or function call? *)
      ( match e with
      | Expr_Var a -> (
          let tys = List.map (function _, ty -> ty) ss' in
          let getters = GlobalEnv.getFuns (Env.globals env) a in
          let ogetters = chooseFunction (Env.globals env) loc "function" (Ident.to_string a) true tys getters in
          match ogetters with
  ...
  | Expr_Record (tc, args, fas) ->
      ...
      let tc' = GlobalEnv.renameType (Env.globals env) tc in
      let args = List.map (function
          | (ov, Expr_Slice (Slice_Single a)) -> (ov, a)
          | arg -> arg
          )
          args
      in
  ...
  | Expr_In (e, p) ->
      ...
      let lets = ref [] in
      let e'' = mk_expr_safe_to_replicate lets e' ety' in
      (mk_let_exprs !lets (Expr_In (e'', p')), type_bool)
  | Expr_Var v ->
      ...
      | None ->
        ...
        | Some (Type_Record ([], fs)) ->
          ...
          let v' = GlobalEnv.renameType (Env.globals env) v in
          (Expr_Record (v', [], []), Type_Constructor (v', []))
        | Some _ ->
          raise (IsNotA (loc, "record or exception type", Ident.to_string v));
        | None ->
          let getters = GlobalEnv.getFuns (Env.globals env) v in
          ( match
            chooseFunction (Env.globals env) loc "function"
          ...
          | None ->
              raise (UnknownObject (loc, "variable or nullary functions", Ident.to_string v))
          )
        )
      )
  | Expr_UApply (e, use_array_syntax, args, throws) ->
      if use_array_syntax then (
        let ss = List.map (function
            | (nm, Expr_Slice s) -> s
            | (nm, arg) -> Slice_Single arg
            )
            args
        in
        tc_expr env loc (Expr_Slices (Isa_utils.type_unknown, e, ss))
      ) else (
        let f = ( match e with
                | Expr_Var f -> f
                | _ ->
                    let msg = Format.asprintf "syntax error in apparent function call `%a`"
                        FMT.expr x
                    in
                    raise (TypeError (loc, msg))
                )
        in
        let args0 = List.map (function
            | (nm, Expr_Slice (Slice_Single arg)) -> (nm, arg)
            | (nm, Expr_Slice s) ->
                let msg = Format.asprintf "function `%a` does not accept slice arguments such as `%a`"
                    FMT.funname f
                    FMT.slice s
                in
                raise (TypeError (loc, msg))
            ...
            args
        in
        let args' = tc_args env loc args0 in
        let (fty, es) = tc_apply env loc "function" f args' in
        (apply_with_runtime_checks loc fty es throws, fty.rty)
      )
  | Expr_Tuple es ->
  ...
  | Expr_Unop (op, e) ->
      ...
      if Ident.equal fty.name Builtin_idents.neg_int
      then (mk_neg_int e', fty.rty)
      else (Expr_TApply (fty.name, fty.parameters, [ e' ], NoThrow), fty.rty)
  | Expr_Unknown t ->
  ...
  | Expr_Slice s ->
      let (s', _) = tc_slice env loc s in
      let width = slice_width s' in
      (Expr_Slice s', type_slice width)
  | Expr_Lit (VInt _)    -> (x, Type_Integer (Some([Set_Single x])))
...
and tc_constraint (env : Env.t) (loc : Loc.t) (c : AST.set_range) : AST.set_range =
  match c with
  | Set_Single e ->
      let e' = check_expr env loc type_integer e in
      Set_Single e'
  | Set_Range (lo, hi) ->
      let lo' = Option.map (check_expr env loc type_integer) lo in
      let hi' = Option.map (check_expr env loc type_integer) hi in
      Set_Range (lo', hi')

and tc_constraints (env : Env.t) (loc : Loc.t) (cs : AST.set_range list) : AST.set_range list =
...
and tc_lexpr2 (env : Env.t) (loc : Loc.t)
  ...
  | LExpr_Var v -> (
      ...
      | None -> (
          let getters = GlobalEnv.getFuns (Env.globals env) v in
          let setters = GlobalEnv.getSetterFun (Env.globals env) (Ident.add_suffix v ~suffix:"write") in
          let ogetter =
            chooseFunction (Env.globals env) loc "nullary function"
              (Ident.to_string v) false [] getters
          in
          match ogetter with
          | Some fty ->
  ...
  | LExpr_Slices (_, e, ss) -> (
       ...
       * Start by testing for assignment function
      ...
      | LExpr_Var a -> (
          let tys = List.map (function _, ty -> ty) ss' in
          let getters = GlobalEnv.getFuns (Env.globals env) a in
          let setters = GlobalEnv.getSetterFun (Env.globals env) (Ident.add_suffix a ~suffix:"write") in
          let ogetters =
            chooseFunction (Env.globals env) loc "function"
              (Ident.to_string a) true tys getters
          in
          let osetters =
            chooseFunction (Env.globals env) loc "assignment function"
          ...
          | None, Some _ ->
              raise (UnknownObject (loc, "function", Ident.to_string a))
          | Some _, None ->
              raise (UnknownObject (loc, "assignment function", Ident.to_string a))
...
let rec tc_lexpr (env : Env.t) (loc : Loc.t)
  ...
  | LExpr_Slices (_, e, ss) ->
      ...
      (* variable slice or setter call?
       * Start by testing for assignment function
       * If that fails, test for slice of a nullary function
       * If that fails, test for an array variable or bitvector variable
       *)
      let e', ty' =
        match e with
        | LExpr_Var a -> (
            ...
            | _ -> (
                ...
                | None, Some _ ->
                    raise (UnknownObject (loc, "nullary function", Ident.to_string a))
                | Some _, None ->
                    raise (UnknownObject (loc, "assignment function", Ident.to_string a))
...
and tc_stmt (env : Env.t) (x : AST.stmt) : AST.stmt list =
  ...
  | Stmt_Assign (l, r, loc) ->
      ...
      let lets' = Isa_utils.mk_assigns loc !lets in
      let asserts' = Isa_utils.mk_assert_stmts !asserts in
      if !verbose then
  ...
  | Stmt_For (v, ty, start, dir, stop, b, loc) ->
      ...
      let ty'' =
        ( match (ty', dir) with
        | (Type_Integer(None), Direction_Up) -> Type_Integer (Some [Set_Range (Some start', Some stop')])
        | (Type_Integer(None), Direction_Down) -> Type_Integer (Some [Set_Range (Some stop', Some start')])
        | _ -> ty'
        )
      in
      let b' =
...
let tc_parameter (env : Env.t) (loc : Loc.t)
  ...
  let ty' =
    ( match oty with
    | None -> Type_Integer (Some [Set_Single (Expr_Var arg)])
...
let mangle_setter_name (f : Ident.t) (fty : AST.function_type) : Ident.t =
  if Option.is_some fty.setter_arg then
...
let tc_funtype (env : Env.t) (loc : Loc.t) (fty : AST.function_type) : AST.function_type =
  ...
  let is_not_global (x : Ident.t) : bool =
      Option.is_none (GlobalEnv.getGlobalVar globals x)
      && Utils.is_empty (GlobalEnv.getFuns globals x)
  in
...
let addFunction (env : GlobalEnv.t) (loc : Loc.t) (qid : Ident.t) (fty : AST.function_type) : funtype =
  ...
  | [] ->
      (* not defined yet *)
      (* ISA allows multiple functions to share the same name.
...
let tc_declaration (env : GlobalEnv.t) (d : AST.declaration) :
  ...
  | Decl_Enum (qid, es, loc) ->
      ...
      let deq = Decl_FunType (eq.funname, fty, loc) in
      let dne = Decl_FunType (ne.funname, fty, loc) in
      [ d; deq; dne ]
  | Decl_Var (qid, ty, loc) ->
  ...
  | Decl_FunInstance (qid, ps, loc) ->
      ...
      | [f] ->
          ...
          [ Decl_FunInstance (f.funname, ps, loc) ]
  ...
  | Decl_Config (qid, ty, i, loc) ->
      ...
      [ Decl_Config (qid, ty', i', loc) ]
  | Decl_Use (p1, p2, loc) ->
      (* todo: check p1 exists and that p2 does not exist *)
      []
  )
...
let tc_declarations (env : GlobalEnv.t) ~(isPrelude : bool) ~(sort_decls : bool)
  ...
  (* Collect all the renamings resulting from use clauses *)
  let uses = List.filter_map
    (function Decl_Use(p1, p2, loc) -> Some((p2, p1)) | _ -> None)
    ds
  in
  GlobalEnv.addUses env uses;
  ...
  let pre = if sort_decls then List.rev (Isa_utils.topological_sort pre) else pre in
