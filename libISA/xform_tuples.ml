...
let returnVariables = new Isa_utils.nameSupply "__r"
      ...
      | Stmt_VarDecl (is_constant, AST.DeclItem_Tuple dis, AST.Expr_If (els, e), loc) ->
        let (vs, ds) = List.map (fun di ->
            ...
            | AST.DeclItem_Var (v, Some vty) ->
                ...
                (AST.LExpr_Var v, s)
            | _ ->
                raise (Error.Unimplemented (loc, "tuple var/let-if", (fun fmt -> Isa_fmt.stmt fmt s)))
            )
          )
          dis
          |> List.split
        ...
        let s = cond_assign l' els e loc in
        Visitor.ChangeTo (ds @ Isa_visitor.visit_stmt (self :> Isa_visitor.isaVisitor) s)
...
let xform_decl (d : AST.declaration) : AST.declaration list =
  match d with
  | Decl_FunDefn (f, fty, body, loc) when Isa_utils.isTupleType fty.rty ->
      let tyname = mkReturnTypeName f in
      let tydecl = mkReturnRecord tyname (Isa_utils.tupleTypes fty.rty) loc in
      let rty' = AST.Type_Constructor (tyname, []) in
      let fty' = { fty with rty = rty' } in
      let replacer = new replaceTupleClass (Some tyname) in
      let body' = Isa_visitor.visit_stmts (replacer :> Isa_visitor.isaVisitor) body in
      let d' = AST.Decl_FunDefn (f, fty', body', loc) in
      [tydecl; d']

  | Decl_FunType (f, fty, loc) when Isa_utils.isTupleType fty.rty ->
      let tyname = mkReturnTypeName f in
      let rty' = AST.Type_Constructor (tyname, []) in
      let fty' = { fty with rty = rty' } in
