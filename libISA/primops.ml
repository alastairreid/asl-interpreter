(****************************************************************
 * ISA primitive types and operations
 *
...
let prim_pdiv_int (x : bigint) (y : bigint) : bigint = assert (Z.sign x >= 0 && Z.sign y > 0); Z.div x y
let prim_prem_int (x : bigint) (y : bigint) : bigint = assert (Z.sign x >= 0 && Z.sign y > 0); Z.rem x y
...
let prim_cdiv_int (x : bigint) (y : bigint) : bigint = Z.cdiv x y
...
let prim_crem_int (x : bigint) (y : bigint) : bigint =
  Z.sub x (Z.mul y (Z.cdiv x y))
...
let prim_pow2_bits (x : bigint) (n : bigint): bitvector = mkBits (Z.to_int n) (Z.shift_left Z.one (Z.to_int x))
...
let prim_crem_sintN (x : sintN) (y : sintN) : sintN =
  assert (x.n = y.n);
  mksintN x.n (Z.sub x.v (Z.mul y.v (Z.cdiv x.v y.v)))
...
let prim_align_sintN (x : sintN) (y : sintN) : sintN =
  assert (x.n = y.n);
  mksintN x.n (Z.mul y.v (Z.div x.v y.v))
...
(** These are not part of the official ISA language but they are
