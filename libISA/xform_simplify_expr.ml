 ...
 *    collected together.
 *
 * 3) Converting the result back to an ISA expression.
 *)
...
let monomial_to_expr (x : monomial) : AST.expr =
  ...
  | Right (c, vs) ->
    List.map (fun v -> AST.Expr_Var v) vs
      |> Isa_utils.mk_mul_ints (Isa_utils.mk_litbigint c)
  )
...
let fmt_mono (fmt : Format.formatter) (x : monomial) : unit =
  ( match x with
  | Left e ->
    Format.fprintf fmt "'%a'" Isa_fmt.expr e
  | Right (c, vs) ->
    Format.fprintf fmt "%s * [%a]" (Z.to_string c) Isa_fmt.varnames vs
  )
...
let neg_mono (x : monomial) : monomial =
  ( match x with
  | Left e ->
    Either.Left (Isa_utils.mk_neg_int e)
  | Right (c, vs) ->
...
let mul_mono (x1 : monomial) (x2 : monomial) : monomial =
  ...
  | (Left e1, _) -> Left (Isa_utils.mk_mul_int e1 (monomial_to_expr x2))
  | (_, Left e2) -> Left (Isa_utils.mk_mul_int (monomial_to_expr x1) e2)
  )
...
let mul_poly (p : polynomial) (q : polynomial) : polynomial =
  if is_pure_poly p && is_pure_poly q then begin
    ...
    mk_poly_from_expr (Isa_utils.mk_mul_int e1 e2)
  end
...
let mk_add_int (x : AST.expr) (y : AST.expr) : AST.expr =
  simplify (Isa_utils.mk_add_int x y)

(* Add ints and simplify - this pattern is surprisingly common *)
let mk_add_ints (ws : AST.expr list) : AST.expr =
  simplify (Isa_utils.mk_add_ints ws)
