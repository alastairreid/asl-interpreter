...
let fv_expr (x : expr) : IdentSet.t =
  ...
  ignore (visit_expr (fv :> isaVisitor) x);
  fv#vars

let fv_type (x : ty) : IdentSet.t =
  let fv = new freevarClass in
  ignore (visit_type (fv :> isaVisitor) x);
  fv#vars
...
let fv_stmts stmts =
  ...
  ignore (visit_stmts (fvs :> isaVisitor) stmts);
  fvs#vars

let fv_decl decl =
  let fvs = new freevarClass in
  ignore (visit_decl (fvs :> isaVisitor) decl);
  fvs#vars
...
let assigned_vars_of_stmts stmts =
  ...
  ignore (visit_stmts (avs :> isaVisitor) stmts);
  avs#result

let assigned_vars_of_decl decl =
  let avs = new assignedVarsClass in
  ignore (visit_decl (avs :> isaVisitor) decl);
  avs#result
...
let subexprs_of_expr (x : expr) : expr list =
  ...
  ignore (visit_expr (subs :> isaVisitor) x);
  subs#result
...
let locals_of_stmts stmts =
    ...
    ignore (Visitor.mapNoCopy (visit_stmt (lc :> isaVisitor)) stmts);
    lc#locals

let locals_of_decl decl =
    let lc = new localsClass in
    ignore (Visitor.mapNoCopy (visit_decl (lc :> isaVisitor)) decl);
    lc#locals
*)
...
let types_of_expr expr =
  ...
  ignore (visit_expr (cc :> isaVisitor) expr);
  cc#result

let types_of_stmts stmts =
  let cc = new typesClass in
  ignore (visit_stmts (cc :> isaVisitor) stmts);
  cc#result

let types_of_decl decl =
  let cc = new typesClass in
  ignore (visit_decl (cc :> isaVisitor) decl);
  cc#result
...
let reachable_decls (roots : Ident.t list) (ds : declaration list) :
  ...
  let next (d : declaration) : IdentSet.t =
    let fvs = new freevarClass in
    ignore (visit_decl (fvs :> isaVisitor) d);
  ...
  object
    ...
    method! vexpr e =
      ...
      | Expr_TApply (f, _, _, throws) ->
          ...
          if Ident.equal f ram_read then begin
    ...
    method! vstmt s =
      ...
      | Stmt_TCall (f, _, _, throws, _) ->
          functions_called <- IdentSet.add f functions_called;
          if throws <> NoThrow then throws_exceptions <- true;
          if Ident.in_list f [ram_init; ram_write] then begin
...
let identify_impure_funs (isConstant : Ident.t -> bool)
  ...
  let is_impure_prim (d : declaration) : bool =
    ( match d with
    | Decl_FunType (f, fty, loc) -> fty.is_builtin && isImpurePrim f
    | _ -> false
    )
  in
  ...
  List.iter (fun d ->
    ( match d with
    | Decl_FunType (f, fty, _) when not fty.is_builtin && not (IdentSet.mem f defined) ->
...
let type_slice (n : expr) = Type_Constructor (slice_ident, [n])
...
let mk_neg_int (x : AST.expr) : AST.expr =
  ( match x with
  | Expr_Lit (VInt i) -> Expr_Lit (VInt (Primops.prim_neg_int i))
  | Expr_Lit (VIntN i) -> Expr_Lit (VIntN (Primops.prim_neg_sintN i))
  | _ -> mk_unop neg_int [] x
  )
...
(** Construct "sign_extend_bits{w, n}(x, n)" *)
let mk_sign_extend_bits (w : AST.expr) (n : AST.expr) (x : AST.expr) : AST.expr =
  if w = n then
    x
  else
...
let mk_concat (szs : AST.expr list) (xs : AST.expr list) : AST.expr =
  let flatten (sz : AST.expr) (x : AST.expr) : (AST.expr list * AST.expr list) =
    ( match x with
    | Expr_Concat(sizes, args) -> (sizes, args)
    | _ -> ([sz], [x])
    )
  in
  let (szss, xss) = List.split (List.map2 flatten szs xs) in
  Expr_Concat (List.concat szss, List.concat xss)
