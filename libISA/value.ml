...
let rec pp_value (fmt : Format.formatter) (x : value) : unit =
  ...
  | VSlice (lo, wd) ->
      Format.fprintf fmt "%s +: %s"
        (prim_cvt_int_decstr lo)
        (prim_cvt_int_decstr wd)
  | VString s ->
...
let _ = begin
  ...
  Flags.registerFlag "trace:local_read" enable_trace_local_read "ISA trace: local variable reads";
  Flags.registerFlag "trace:local_write" enable_trace_local_write "ISA trace: local variable writes";
  Flags.registerFlag "trace:primop" enable_trace_primops "ISA trace: calls to builtin functions";
  Flags.registerFlag "trace:function" enable_trace_functions "ISA trace: calls to functions";
end
...
let eval_prim (f : Ident.t) (tvs : value list) (vs : value list) : value option =
  ...
  | [], [ VInt x; VInt y ] when Ident.equal f pdiv_int ->
      Some (VInt (prim_pdiv_int x y))
  | [], [ VInt x; VInt y ] when Ident.equal f prem_int ->
      Some (VInt (prim_prem_int x y))
  ...
  | [], [ VInt x; VInt y ] when Ident.equal f cdiv_int ->
      Some (VInt (prim_cdiv_int x y))
  | [], [ VInt x; VInt y ] when Ident.equal f crem_int ->
      Some (VInt (prim_crem_int x y))
  ...
  | [_], [ VIntN x; VIntN y ] when Ident.equal f cdiv_sintN ->
      Some (VIntN (prim_cdiv_sintN x y))
  | [_], [ VIntN x; VIntN y ] when Ident.equal f crem_sintN ->
      Some (VIntN (prim_crem_sintN x y))
  ...
  | [_], [ VInt x; VInt n ] when Ident.equal f pow2_bits -> Some (VBits (prim_pow2_bits x n))
  ...
  | [VInt a; VInt n], [ VRAM ram; VBits i; _; VBits x ] when Ident.equal f ram_write ->
      Some (prim_write_ram a n ram i.v x; VTuple [])
