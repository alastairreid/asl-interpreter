...
(** ISA to C runtime library support (fallback) *)
...
module Runtime : RT.RuntimeLib = struct
  ...
  let cdiv_sintN (fmt : PP.formatter) (n : int) (x : RT.rt_expr) (y : RT.rt_expr) : unit =
    PP.fprintf fmt "({ %a __tmp1 = %a; "
      ty_sint n
      RT.pp_expr x;
    PP.fprintf fmt "%a __tmp2 = %a; "
  ...
  let crem_sintN (fmt : PP.formatter) (n : int) (x : RT.rt_expr) (y : RT.rt_expr) : unit =
    PP.fprintf fmt "({ %a __tmp1 = %a; "
      ty_sint n
      RT.pp_expr x;
    PP.fprintf fmt "%a __tmp2 = %a; "
  ...
  let cdiv_int (fmt : PP.formatter) (x : RT.rt_expr) (y : RT.rt_expr) : unit = cdiv_sintN fmt int_width x y
  let crem_int (fmt : PP.formatter) (x : RT.rt_expr) (y : RT.rt_expr) : unit = crem_sintN fmt int_width x y
    ...
    if w <= 64 then
      ...
      if top_hex_chars > 0 then
        add (Printf.sprintf "%%0%dllx" top_hex_chars)
          (fun fmt -> PP.fprintf fmt "(long long)(%a >> %d)" RT.pp_expr arg (full_limbs * 64));
      for i = full_limbs - 1 downto 0 do
        add "%016llx"
          (fun fmt ->
            if i = 0 then
              PP.fprintf fmt "(long long)%a" RT.pp_expr arg
            else
              PP.fprintf fmt "(long long)(%a >> %d)" RT.pp_expr arg (i * 64))
      done
    end
