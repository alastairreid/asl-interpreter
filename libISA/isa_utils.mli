...
val subst_var : AST.expr Bindings.t -> Isa_visitor.access_kind -> Ident.t -> Ident.t
...
(** More flexible substitution class - takes a function instead
    of a binding set.
 *)
class substFunClass : (Ident.t -> AST.expr option) -> Isa_visitor.isaVisitor
...
class replaceExprClass : (AST.expr -> AST.expr option) -> Isa_visitor.isaVisitor
...
val type_slice : AST.expr -> AST.ty
val type_sintN : AST.expr -> AST.ty
val type_unknown : AST.ty

val type_of_value : Loc.t -> Value.value -> AST.ty
...
(** Construct "Concat(a,b,c,d)" *)
val mk_concat : AST.expr list -> AST.expr list -> AST.expr
