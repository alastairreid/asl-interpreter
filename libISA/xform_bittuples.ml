...
let assign_var = new Isa_utils.nameSupply "__a"
...
let xform_dbi
    ...
    : AST.stmt list =
    let tmp_ident = assign_var#fresh in
    let widths = List.map (fun (_, ty) -> Option.get (Isa_utils.width_of_type ty)) dbs in
    let total_width = Xform_simplify_expr.mk_add_ints widths in
    let tmp_ty = Isa_utils.type_bits total_width in
    let tmp_var = DeclItem_Var (tmp_ident, Some tmp_ty) in
    let tmp_const_decl = Stmt_VarDecl (true, tmp_var, r, loc) in

    let (ss, _) = List.fold_right (fun (ov, ty) (ss, idx) ->
      let w = Option.get (Isa_utils.width_of_type ty) in
    ...
    ) dbs ([], Isa_utils.zero) in

    tmp_const_decl :: ss

let slice_width (x : AST.slice) : AST.expr =
  ( match x with
  | Slice_Single e -> Isa_utils.one
  | Slice_HiLo (hi, lo) -> Xform_simplify_expr.mk_add_int (Isa_utils.mk_sub_int hi lo) Isa_utils.one
  | Slice_LoWd (lo, wd) -> wd
  | Slice_HiWd (hi, wd) -> wd
  ...
  object (self)
    ...
    method! vexpr x =
      ...
      | Expr_Slices (ty, e, ss) when List.length ss > 1 -> (* todo: only if e is atomic *)
        ...
        let x' = Isa_utils.mk_expr_safe_to_replicate assign_var e ty (fun e' ->
...
let xform_stmts (ss : AST.stmt list) : AST.stmt list =
  ...
  Isa_visitor.visit_stmts (replacer :> Isa_visitor.isaVisitor) ss

let xform_decls (ds : AST.declaration list) : AST.declaration list =
  let replacer = new replace_bittuples (Some ds) in
  List.map (Isa_visitor.visit_decl (replacer :> Isa_visitor.isaVisitor)) ds
