...
let catch_labels = new Isa_utils.nameSupply "catch"
...
(* Helper functions *)
let helper_functions = Identset.mk_bindings [
    (asl_fuzz, "ASL_fuzz");
    (asl_end_execution, "ASL_end_execution");
...
(* special case: to suppress a clang warning message when generating '(a == b) | (c == d)', we cast to int first *)
and bool_binop (loc : Loc.t) (fmt : PP.formatter) (op : string) (x : AST.expr) (y : AST.expr) : unit =
  let cast_expr fmt x = PP.fprintf fmt "((int)("; expr loc fmt x; PP.fprintf fmt "))"
  in
  PP.fprintf fmt "(%a %s %a)"
    cast_expr x
    op
...
and funcall (loc : Loc.t) (fmt : PP.formatter) (f : Ident.t) (tes : AST.expr list) (args : AST.expr list) : unit =
  ...
  | ([], [x;y]) when Ident.equal f strict_and_bool -> bool_binop loc fmt "&" x y
  | ([], [x;y]) when Ident.equal f strict_or_bool  -> bool_binop loc fmt "|" x y
  ...
  | ([], [x;y]) when Ident.equal f pdiv_int -> Runtime.zdiv_int fmt (mk_expr loc x) (mk_expr loc y)
  | ([], [x;y]) when Ident.equal f prem_int -> Runtime.zrem_int fmt (mk_expr loc x) (mk_expr loc y)
  ...
  | ([], [x;y]) when Ident.equal f cdiv_int -> Runtime.cdiv_int fmt (mk_expr loc x) (mk_expr loc y)
  | ([], [x;y]) when Ident.equal f crem_int -> Runtime.crem_int fmt (mk_expr loc x) (mk_expr loc y)
  ...
  | ([n],   [x;_]) when Ident.equal f pow2_bits ->
                   let t fmt = Runtime.bits_literal fmt (Primops.mkBits n (Z.one)) in
                   Runtime.lsl_bits fmt n t (mk_expr loc x)
  ...
  | ([a],   [ram;v])      when Ident.equal f ram_init  -> Runtime.ram_init  fmt a (mk_expr loc ram) (mk_expr loc v)
  | ([a;n], [ram;addr;_]) when Ident.equal f ram_read  -> Runtime.ram_read  fmt a n (mk_expr loc ram) (mk_expr loc addr)
  | ([a;n], [ram;addr;_;v]) when Ident.equal f ram_write -> Runtime.ram_write fmt a n (mk_expr loc ram) (mk_expr loc addr) (mk_expr loc v)
  ...
  | ([], args) when Identset.Bindings.mem f helper_functions ->
  ...
  *)
  | ([size], [x;_]) when Ident.equal f uint_to_bits -> Runtime.ffi_c2asl_bits_small size fmt (mk_expr loc x)
  | ([size], [x;_]) when Ident.equal f uint_from_bits -> Runtime.ffi_asl2c_bits_small size fmt (mk_expr loc x)
  | ([size], [x;_]) when Ident.equal f sint_to_integer -> Runtime.ffi_c2asl_sintN_small size fmt (mk_expr loc x)
  | ([size], [x;_]) when Ident.equal f sint_from_integer -> Runtime.ffi_asl2c_sintN_small size fmt (mk_expr loc x)
  | ([],     [x])   when Ident.equal f int_to_integer -> Runtime.ffi_c2asl_integer_small fmt (mk_expr loc x)
  | ([],     [x])   when Ident.equal f int_from_integer -> Runtime.ffi_asl2c_integer_small fmt (mk_expr loc x)
...
and varty ?(const_ref = false) (loc : Loc.t) (fmt : PP.formatter) (v : Ident.t) (x : AST.ty) : unit =
  ...
  | Type_Constructor (tc, []) when Ident.equal tc boolean_ident ->
    PP.fprintf fmt "bool %a" ident v
  | Type_Constructor (tc, []) when Ident.equal tc string_ident ->
    PP.fprintf fmt "const char *%a" ident v
...
let stmt_line_info (fmt : PP.formatter) (x : AST.stmt) : unit =
  if !include_line_info then
    ( match Isa_utils.stmt_loc x with
    | Range (p, _) ->
...
let declaration (fmt : PP.formatter) ?(is_extern : bool option) (x : AST.declaration) : unit =
  ...
  vbox fmt (fun _ ->
      ...
      | Decl_FunType (f, fty, loc) ->
          if not fty.is_builtin then begin
            function_header loc fmt f fty;
            PP.fprintf fmt ";@,@,"
          end
...
let type_decls (xs : AST.declaration list) : AST.declaration list =
  let mk_type_decl (x : AST.declaration) : AST.declaration option =
    ...
    | Decl_FunType (_, fty, _) when not fty.is_builtin
      -> Some x
