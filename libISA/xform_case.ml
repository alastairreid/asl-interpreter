...
let rec match_pattern (loc : Loc.t) (e : AST.expr) (p : AST.pattern) : AST.expr =
  ...
  | (Pat_Lit (VBits b as v), _) -> mk_eq_bits (Isa_utils.mk_litint b.n) e (Expr_Lit v)
  ...
  | _ ->  raise (Error.Unimplemented (loc, "match_pattern", fun fmt -> Isa_fmt.pattern fmt p))
  )
      ...
      | Stmt_Case (e, oty, alts, odefault, loc)
        ...
        let default = match odefault with
          ...
          | None ->
            ...
            let s = AST.Stmt_TCall (f, [], [Isa_utils.mk_litstr msg], NoThrow, loc) in
            ([s], loc)
        in
        let ss = mk_if branches default loc in
        let ss' = Isa_visitor.visit_stmts (self :> Isa_visitor.isaVisitor) ss in
        Visitor.ChangeTo ss'
...
let xform_stmts (x : AST.stmt list) : AST.stmt list =
  let simplify = new simplifyCaseClass in
  Isa_visitor.visit_stmts (simplify :> Isa_visitor.isaVisitor) x

let xform_decl (x : AST.declaration) : AST.declaration =
  let simplify = new simplifyCaseClass in
  Isa_visitor.visit_decl (simplify :> Isa_visitor.isaVisitor) x
