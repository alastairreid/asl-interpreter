...
let range_of_constraint (x : AST.set_range) : range =
  ( match x with
  | Set_Single y -> range_of_expr y
  | Set_Range (Some lo, Some hi) -> union_range (range_of_expr lo) (range_of_expr hi)
  | _ -> None
  )

let range_of_constraints (x : AST.set_range list) : range =
...
let mk_cvt_sintN_bits (target_size : AST.expr) (e : AST.expr) : AST.expr option =
  let r = range_of_expr e in
  Option.map (fun b ->
    let source_size = int_of_bounds b in
    AST.Expr_TApply (cvt_sintN_bits, [expr_of_int source_size; target_size], [mk_cvt_int_sintN source_size e; target_size], NoThrow)
  ) r
...
class boundedClass = object (self)
  ...
  method! vexpr x =
    ...
    | Expr_Let (v, ty, e1, e2) ->
        ...
        | Some b ->
            ...
            let e2' = Isa_visitor.visit_expr (self :> Isa_visitor.isaVisitor) e2 in
            self#sub_lrange v;
    ...
    | Expr_TApply (f, ps, args, can_throw) ->
        ...
        | Some fty ->
            ...
            let args' = List.map (Isa_visitor.visit_expr (self :> Isa_visitor.isaVisitor)) args in
  ...
  val mutable return_type : AST.ty = Isa_utils.type_unit
  ...
  method vdecls (ds : AST.declaration list) : AST.declaration list =
    ...
    Visitor.mapNoCopy (Isa_visitor.visit_decl (self :> Isa_visitor.isaVisitor)) ds
end
